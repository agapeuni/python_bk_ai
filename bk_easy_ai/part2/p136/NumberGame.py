...
# A팀은 룰을 만들고, 학습용 및 테스트용 데이터셋을 제공 후, B, C팀에게 정답을 요구
# B팀은 학습용 데이터셋을 이용하여 룰을 찾고, 테스트용 데이터의 정답을 맞힘   
# C팀은 학습용 데이터셋을 이용하여 AI학습을 하고, 테스트용 데이터의 정답을 예측
# B팀, C팀중 먼저 맞춘 팀이 승리, 못 맞추면 데이터/정답셋을 추가적으로 요구
# 승패가 결정나면 A, B, C팀의 역할을 변경하여 다시 게임시작
...
epoch = 80000       # 학습 에포크(학습 횟수)
n_hidden = 5        # 은닉층 노드수
min_error = 0.0001  # 최소 오차값 설정: 오차가 이보다 작아지면 학습정지
...
def train(X, T):
    ...
    for n in range(epoch):                  # epoch수 만큼 반복
        ...
        if error[n] < min_error: # 오차값이 설정치보다 작아지면 학습을 중단
            break
...
##################### 학습용 입력 데이터 및 정답 데이터 #####################
    X = np.array([[1,2,3], [1,2,7], [2,3,7],   # 입력값: A팀 제공
                  [3,4,6], [3,5,7], [3,7,8],   # 0에서 9사이 임의의 숫자
                  [5,6,7], [5,7,9], [5,8,9], 
                  [6,7,8], [6,8,9], [7,8,9]])      
    T = np.array([[-2,1],  [-6,5],  [-4,5],    # 출력값(교시값): A팀 제공
    ...
    ############ 테스트용 입력 데이터 입력 및 예측 ###############
    test = np.array([4,5,9])              # Test용 입력 데이터: A팀 제공
    ...
    # 게임을 할 경우는 밑의 줄(209라인)을 전체 삭제할 것 (이유: 계산 룰을 모르기 때문에)
