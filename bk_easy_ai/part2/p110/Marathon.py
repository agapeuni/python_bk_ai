# 마라톤에서 좋은 기록을 얻기 위한 예측 프로그램
# 초기 속도에 따른 최종 기록을 예측
# 입력은 처음 15km구간 평균 속도, 출력은 다음 구간의 속도 감소와 최종 기록 
# 1X3X3의 인공 신경망 만들기
...
# 입력(input)값과 정답(teaching data) 데이터 저장하기
...
for n in range(1, epoch+1): # 1부터 epoch까지 반복
    for i in range(len(input_data)): 
        ...
        t1 = teaching_data[i][0]/15 # i번째 행의 첫번째 숫자/첫번째 구간 속도 감소(15로 나누어 정규화)
        t2 = teaching_data[i][1]/15 # i번째 행의 두번째 숫자/두번째 구간 속도 감소(15로 나누어 정규화)
        t3 = teaching_data[i][2]-2  # i번째 행의 세번째 숫자/최종 기록(빼기2로 정규화)
        ########## 순방향 계산 #########
