# LSTM에 의한 환율 예측 프로그램(원-달러화 환율 사이트에서 가지고 옴)
...
# print(len(train), len(test))  # 트레인셋과 테스터셋의 길이를 확인해 보기

# 데이터셋 배열로 변환(convert an array of values into a dataset matrix)
def create_dataset(dataset, maxlen):    # maxlen은 다음 시간 영역 예측을 위한 앞쪽 시간대의 스텝수
...
maxlen = 3   # 스텝 수를 20정도로 늘리면 어떻게 바뀔까? 다양하게 테스트해 보기
...
print(trainX[:10,:])    # 데이터셋의 형태가 어떻게 바뀌었는지 확인해 보기
...
model.add(Dense(1))
model.compile(loss='mean_squared_error', optimizer='adam') # 디폴트 활성화 함수: 시그모이드
...
# invert predictions
# 예측된 데이터를 원래의 배열 형태로 바꾸고, inverse_transform()으로 오차 계산을 함.
