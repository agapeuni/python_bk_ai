# Mediapipe에서 제공하는 Hands 함수를 통해 손의 21개 랜드마크를 획득
# MCP: 밑쪽 관절, PIP: 중간 관절, DIP: 끝쪽 관절, CMC: 엄지 뿌리관절
...
mpDraw = mp.solutions.drawing_utils  # 영상 위에 그림을 그려 주기 위한 모듈
...
while True:
    ...
    results = hands.process(imgRGB)  # RGB 영상으로 바꾸고 hands 모듈 수행

    if results.multi_hand_landmarks:    # 두 개의 손의 랜드마크가 잡힌다면
    ...
    # 초당 프레임 수를 표시하고 싶다면 아래의 주석문 4줄을 해제(ctl+/)하세요.  
    # cTime = time.time()     # time 함수를 써서 현재 시간 체크
    # fps = 1/(cTime-pTime)   # 현재 프레임과 전 프레임의 시간 간격으로 fps계산 
    # pTime = cTime           # 현재 시간을 전 시간으로 설정
